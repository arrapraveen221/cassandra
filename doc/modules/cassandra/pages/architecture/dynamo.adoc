...
however, adding a single node might invalidate almost all the
mappings.
...
In the same way, when a single node is initialized in a completely new rack,
it will be regarded as a replica for the entire ring. For this reason, many
...
Transient replication is an experimental feature in Cassandra 4.0 version not
...
supported. You also can't use `LWT`, logged batches, or counters in 4.0 version.
...
written to hints). When the node resumes sending heartbeats again, Cassandra
