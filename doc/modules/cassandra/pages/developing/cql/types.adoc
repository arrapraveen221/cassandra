...
* Counters do not support xref:cassandra:developing/cql/functions.adoc#writetime-and-ttl-functions[expiration].
...
as https://en.wikipedia.org/wiki/Unix_time[the epoch]: January 1st, 1970 at
00:00:00 GMT.
...
the values below are valid `timestamp` values for Mar 2, 2011, at
04:05:00 AM, GMT:
...
and their values can be inputted using collection literals:
...
Collections are meant for storing/de-normalizing relatively small amount
...
timeout problematic. Therefore, it is recommended to use sets instead of lists whenever possible.

It is an anti-pattern to store large amounts of data in a single collection.
...
purpose, a value of a custom type is the same as that of a `blob`, and
