...
A table is always associated with a keyspace, and can be specified
in a fully-qualified manner by including the keyspace name. If the table name is not fully-qualified,
it is assumed to reside in the _current_ keyspace.
...
A `column_definition` is comprised of the name of the column and its xref:cassandra:developing/cql/ddl.adoc#column-definition[type],
...
rows in the partition (`pk` is the partition key, and both
...
| `compaction` |_map_ |_see below_ | xref:cassandra:managing/operating/compaction/index.adoc#compaction-options[Compaction options]
| `compression` |_map_ |_see below_ | xref:cassandra:managing/operating/compression.adoc#compression-options[Compression options]
...
|===
|Option |Default |Description
| `keys` | ALL | Whether to cache keys in the key cache for this table, valid values are: `ALL` and `NONE`.
...
* Use `WITH` to change a table option. The xref:cassandra:developing/cql/ddl.adoc#create-table-options[options]
are the same as those when creating a table, except for `CLUSTERING ORDER`.
