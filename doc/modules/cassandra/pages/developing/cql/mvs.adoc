...
* the selection is limited to those that only
...
Further, xref:cassandra:developing/cql/ddl.adoc#static-column[static columns] cannot be included in a materialized view.
...
creating a MV allows the same options as xref:cassandra:reference/cql-commands/create-table.adoc[table creation].
...
The options that can be updated are the same as
xref:cassandra:reference/cql-commands/alter-table.adoc[alter table].
If the view does not exist, the statement will return an error, unless `IF EXISTS` is used in which case the operation will have no effect.
