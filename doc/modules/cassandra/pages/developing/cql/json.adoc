...
By default, or when DEFAULT NULL is explicitly specified, a column that is not
included in the JSON map will be set to NULL. This means that any existing value for
that column is deleted with a tombstone.
If the `DEFAULT UNSET` directive is used after
the value, omitted column values are unset, and any
...
types. For example, floats, integers, UUIDs, and dates can be represented by
