...
now reserved keywords. In the unlikely case you are using them as a
...
valid `blob` value (in which case 42 was interpreted as hexadecimal
representation of the blob). This is no longer true; type validation
for constants has become more stringent. See the `data types <data-types>`
